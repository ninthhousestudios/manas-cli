<!-- opencode-specific manas instructions. Appended to the shared manas
     instructions when `manas warm opencode` boots a session. Edit the seeded
     copy at ~/.manas/opencode-instructions.md to change this without a
     rebuild. -->
...
- Subject: imperative mood, no trailing period, ≤ ~70 chars, naming what the commit *does* ("Write opencode config into a manas-owned dir", not "fixed config"). Not "fixed", "changes", or "stuff".
