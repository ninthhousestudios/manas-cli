<sutra_mcp>
sutra (MCP code intelligence) is the *required* path for code here, not just preferred: the guard denies built-in Glob/Grep/Read on code, so use the sutra equivalent. Built-in Glob/Grep/Read are for non-code content only. The per-tool decision rules and the batch-load-on-deferral line live in sutra's own MCP `instructions` string (it survives tool deferral); this block covers only what that string can't.

For text search in Bash prefer `rg` (ripgrep) over `grep`: faster, respects `.gitignore`, supports `-g` globbing and `|` alternation. A literal text pattern across file contents is always `rg`, never a sutra symbol lookup.
